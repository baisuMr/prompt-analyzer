...
- **一次性**读取所有 `.jsonl` 文件，解析每行 JSON 到内存数组
- 统计总记录数，确定数据起始月份
- 后续抽样、安全扫描、子代理分发均基于此次读取结果，不再重复读盘
- **注意**：数据统计的中间输出（行数、字段名、月份分布等）不向用户展示，仅在最终报告中体现汇总数字
...
### 5. 并行子代理分析 + 安全扫描（全部并行，不阻塞）

**派发策略**：将 3 个子代理 + 安全扫描共 4 个任务**同时并行派发**，不等待子代理完成再做安全扫描。

**子代理数据精简**：传给子代理的提示词必须精简字段，降低 token 消耗和假死风险：
- **子代理 1（技能）**：只传 `ts`（时间戳）和 `prompt` 文本，按时间排序
- **子代理 2（行为）**：只传 `prompt` 文本 + `session` 会话 ID + `project` 项目路径
- **子代理 3（风格）**：只传 `prompt` 文本列表，不含任何元数据

**进度提示**：4 个任务派发后，仅输出一行状态（如「分析任务已并行启动，等待结果返回…」），之后**禁止轮询**。等待期间不要反复执行空操作或输出「等待中」之类的内容，系统会在子代理完成后自动通知。

**稳定性注意**：若某个子代理超过 3 分钟无响应或输出大小 < 100 字节，应终止并以更精简的数据重试一次（去掉 JSON 格式，改为纯文本列表）。

**数据过滤**：传给子代理之前，需过滤以下非用户输入记录：
- `prompt` 包含 `<task-notification>` XML 标签的（子代理运行结果回写，非用户输入）
- `prompt` 仅为 `[Image #N]` 等图片占位符的

**数据提取约束**：每个子代理必须在输出末尾附带一个 `\`\`\`json` 代码块，包含结构化的分析结果数据。主进程直接从 JSON 代码块提取数据注入 HTML DATA 对象，不要从纯文本中解析。
...
使用步骤 2 已读取的全量数据，用正则模式逐条扫描 prompt 文本。此步骤与子代理分析同时派发，不等待子代理完成。

**扫描时注意**：扫描结果写入临时文件（如 `.tmp/security_scan.json`），**不要**在终端展示原始输出。安全结果仅在最终报告（步骤 8）中汇总呈现。
...
收集 4 个并行任务（3 个子代理 + 安全扫描）的全部结果，去重合并，生成使用建议。

**等待策略**：在等待子代理结果期间，可预先读取 `templates/report.html` 备用，避免后续重复读盘。
...
将以上所有结果整合，以清晰简洁的终端格式输出。

**输出原则**：
- 每个维度用紧凑格式，避免冗余换行
- 评分用数字+简短说明，不展开长篇分析
- 安全和数据统计优先展示
- 输出总长度控制在 100 行以内，不要罗列原始数据
...
- 告知用户时必须输出**完整绝对路径**，不要用相对路径 `./`：
  - Windows 示例：「报告已生成，路径：`E:\mzheng\prompti-profile-report.html`，双击即可在浏览器中打开」
  - Mac/Linux 示例：「报告已生成，路径：`/home/user/project/ai-profile-report.html`，双击即可在浏览器中打开」
...
- **并行优先**：子代理 + 安全扫描共 4 个任务必须一次性并行派发，不串行等待
- **数据精简**：传给子代理的数据必须裁剪字段，只传必需字段，降低 token 和假死风险
- **子代理超时重试**：超过 3 分钟无响应或输出 < 100 字节，终止并以纯文本列表重试
- **一次读盘**：数据只在步骤 2 读取一次，后续全部基于内存数据
- **安全扫描用正则**：不要用 AI 判断，直接在主进程用 JavaScript 正则匹配
- **评分基于数据**：不要凭空打分，必须引用实际提示词作为依据
- **建议具体可操作**：每条建议必须包含可执行的行动，不要泛泛而谈
- **历史导入格式**：必须使用 `type: "last-prompt"` 提取 `lastPrompt` 字段，不可使用 `role: "user"` / `content`
- **抑制辅助输出**：数据统计、安全扫描等 Bash 辅助脚本的输出结果写入临时文件或直接用于后续步骤，**不要**在终端向用户展示。用户只应看到最终分析报告和 HTML 路径，不应看到中间数据处理的原始输出
