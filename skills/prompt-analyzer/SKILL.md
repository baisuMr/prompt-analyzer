...
- 若用户同意：
  ...
  - 对每条记录，提取 `role` 为 `user` 的消息：
    - 提取 `content` 字段作为 prompt 文本
    - 提取原始 `ts`（或 `timestamp`）字段作为记录时间；若 JSONL 中无时间字段，则使用该会话文件的修改时间作为近似值
    - **禁止使用当前时间戳**，否则会破坏成长轨迹和活跃时段分析的准确性
  - 每条记录中 session 字段使用原会话文件名
  - 避免导入与已有记录完全重复的内容（相同 prompt 文本 + 相同时间段 ±5 分钟 跳过）
